...
const RPC_ENDPOINT =
    process.env.RPC_ENDPOINT || "https://mainnet.helius-rpc.com/?api-key=";
const RPC_WEBSOCKET =
    process.env.RPC_WEBSOCKET || "wss://rpc.helius.xyz/?api-key=";
...
function getPublicKey(input: string): string {
    try {
        ...
        try {
            ...
            throw new Error(
                "Input must be either a valid JSON keypair file path or a valid Solana public key",
            );
        }
    }
}
...
const mainFunction = async () => {
    ...
    const myFilteredProfiles = myProfiles.filter((profile) => {
        ...
        return profile.data.profileKeys.some(
            (pk) => pk.key.toBase58() === targetProfileKey,
        );
    });
    ...
    console.log(
        `Found ${fleets.length} fleets for profile ${targetProfile.toBase58()}`,
    );
    for (let index2 = 0; index2 < fleets.length; index2++) {
        ...
        console.log(
            "\nFleet:",
            fleetData.fleetLabel
                ? byteArrayToString(fleetData.fleetLabel)
                : "Unnamed Fleet",
        );
        console.log("Fleet key:", thisFleet.key.toBase58());
        console.log(
            "Ship counts:",
            JSON.stringify(fleetData.shipCounts, null, 2),
        );
        ...
        console.log(
            "Warp cooldown expires at:",
            new Date(warpCooldown * 1000).toLocaleString(),
        );
        console.log(
            "Scan cooldown expires at:",
            new Date(scanCooldown * 1000).toLocaleString(),
        );
        ...
        if (fleetData.state && fleetData.state.name === "MoveOrder") {
            console.log(
                "Moving to:",
                fleetData.state.moveOrder.destination.toBase58(),
            );
            console.log("Movement speed:", fleetData.state.moveOrder.speed);
            console.log(
                "Distance remaining:",
                fleetData.state.moveOrder.distanceRemaining,
            );
        }
        if (fleetData.state && fleetData.state.name === "MiningOrder") {
            console.log(
                "Mining at:",
                fleetData.state.miningOrder.resource.toBase58(),
            );
            console.log("Mining rate:", fleetData.state.miningOrder.miningRate);
            console.log(
                "Resource type:",
                fleetData.state.miningOrder.resourceType,
            );
        }
        ...
        if (fleetData.resources) {
            console.log(
                "Resources:",
                JSON.stringify(fleetData.resources, null, 2),
            );
        }
        if (fleetData.cargo) {
